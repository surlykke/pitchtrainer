...
void MainWindow::newPitch()
{
    ...
    QList<int> availableIntervals;
    for (int i = 0; i < 13; i++) {
        if (intervals[i]) {
            availableIntervals << i;
        }
    }

    if (availableIntervals.size() == 0) {
        QMessageBox mb;
        mb.setText("You must enable at least on intervals in settings...");
        mb.exec();
        return;
    }

    int interval = availableIntervals[rand() % availableIntervals.size()];

    bool descending =  (settings->getExcercise() == 0 && ((rand() % 2) == 0)) ||
                       (settings->getExcercise() == 1);
